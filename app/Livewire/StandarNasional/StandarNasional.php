...
class StandarNasional extends Component
{
    use WithPagination;
    ...
    public function edit($id)
    {
        ...
        if ($standar->level() !== 1) {
            session()->flash('error', 'Hanya standar level 1 yang dapat diedit di halaman ini.');
            return;
        }
        $this->form = [
    ...
    public function confirmDelete($id)
    {
        $standar = StandarNasionalModel::findOrFail($id);
        if ($standar->level() !== 1) {
            session()->flash('error', 'Hanya standar level 1 yang dapat dihapus di halaman ini.');
            return;
        }
    ...
    public function save()
    {
        ...
        $validated = $this->validate([
            'form.standar' => ['required', 'string', 'max:50', 'unique:standar_nasional,standar,' . ($this->standarId ?: 'NULL')],
            ...
            'form.standar.unique' => 'Nama standar sudah ada.',
        ...
        try {
            if ($this->isEditMode) {
                ...
                $standar->update([
                    ...
                    'parent_id' => null, // Level 1 has no parent
                ]);
                session()->flash('success', 'Standar Nasional berhasil diperbarui!');
            } else {
                ...
                session()->flash('success', 'Standar Nasional berhasil ditambahkan!');
            }
    ...
    public function delete()
    {
        try {
            ...
            $standar->delete();
            session()->flash('success', 'Standar Nasional berhasil dihapus!');
            $this->closeDeleteModal();
        } catch (\Exception $e) {
            session()->flash('error', 'Gagal menghapus Standar Nasional: ' . $e->getMessage());
            $this->closeDeleteModal();
        }
    }
    ...
    {
        $standars = StandarNasionalModel::whereNull('parent_id')
        ...
        return view('livewire.standar-nasional.index', [
            'standars' => $standars,
        ...
        ])->layout('components.layouts.app', ['title' => __('Standar Nasional Level 1')]);
    }
}