...
class Level2StandarNasional extends Component
{
    use WithPagination;
    ...
    public function edit($id)
    {
        ...
        if ($standar->level() !== 2) {
            session()->flash('error', 'Hanya standar level 2 yang dapat diedit di halaman ini.');
            return;
        }
        $this->form = [
    ...
    public function confirmDelete($id)
    {
        $standar = StandarNasionalModel::findOrFail($id);
        if ($standar->level() !== 2) {
            session()->flash('error', 'Hanya standar level 2 yang dapat dihapus di halaman ini.');
            return;
        }
    ...
    public function save()
    {
        ...
        try {
            if ($this->isEditMode) {
                ...
                session()->flash('success', 'Standar Level 2 berhasil diperbarui!');
            } else {
                ...
                session()->flash('success', 'Standar Level 2 berhasil ditambahkan!');
            }
    ...
    public function delete()
    {
        try {
            ...
            $standar->delete();
            session()->flash('success', 'Standar Level 2 berhasil dihapus!');
            $this->closeDeleteModal();
        } catch (\Exception $e) {
            session()->flash('error', 'Gagal menghapus Standar Level 2: ' . $e->getMessage());
            $this->closeDeleteModal();
        }
    }
    ...
    public function render()
    {
        ...
        return view('livewire.standar-nasional.level2', [
            'parent' => $parent,
            'standars' => $standars,
        ...
        ])->layout('components.layouts.app', ['title' => __('Standar Nasional Level 2 - ' . $parent->standar)]);
    }
}