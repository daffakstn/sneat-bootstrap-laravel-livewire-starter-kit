@section('title', __('Manajemen Lembaga Akreditasi'))
...
<div class="row g-4">
    <div class="col-lg-12">
        ...
        <!-- Modal Form -->
        <div class="modal fade" id="lembagaModal" tabindex="-1" wire:ignore.self>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ $isEditMode ? __('Edit Lembaga Akreditasi') : __('Tambah Lembaga Akreditasi') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        ...
                        <form wire:submit.prevent="save">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="lembaga" class="form-label fw-medium">{{ __('Lembaga') }} <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('form.lembaga') is-invalid @enderror" 
                                               id="lembaga" wire:model="form.lembaga" placeholder="{{ __('Masukkan nama lembaga') }}">
                                        @error('form.lembaga') 
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ __('Konfirmasi Hapus Lembaga Akreditasi') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        <p>{{ __('Yakin ingin menghapus lembaga akreditasi ini? Tindakan ini tidak dapat dibatalkan.') }}</p>
                    </div>
                    <div class="modal-footer">
        ...
        <!-- Lembaga Akreditasi Table -->
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('Daftar Lembaga Akreditasi') }}</h5>
                <div class="d-flex gap-2">
                    <!-- Search -->
                    <div class="position-relative">
                        <input type="text" 
                               class="form-control" 
                               placeholder="{{ __('Cari lembaga...') }}"
                    ...
                    <button class="btn btn-primary" wire:click="create">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-circle me-2" viewBox="0 0 16 16">
                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16"/>
                            <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4"/>
                        </svg>{{ __('Tambah Lembaga Akreditasi') }}
                    </button>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            @forelse ($lembagas as $index => $lembaga)
                                <tr>
                                    <td class="fw-medium">{{ $lembagas->firstItem() + $index }}</td>
                                    <td>{{ $lembaga->lembaga }}</td>
                                    <td>{{ $lembaga->keterangan ?: '-' }}</td>
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <button class="btn btn-sm btn-primary" wire:click="edit({{ $lembaga->id }})" title="{{ __('Edit') }}">
                                                <i class="bx bx-edit-alt"></i>
                                            </button>
                                            <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $lembaga->id }})" title="{{ __('Hapus') }}">
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        <div class="text-muted">
                                            ...
                                            @else
                                                <p class="mb-0">{{ __('Belum ada lembaga akreditasi yang ditambahkan') }}</p>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
...
document.addEventListener('livewire:init', () => {
    Livewire.on('show-modal', () => {
        const modal = new bootstrap.Modal(document.getElementById('lembagaModal'));
        modal.show();
    });
    
    Livewire.on('hide-modal', () => {
        const modal = bootstrap.Modal.getInstance(document.getElementById('lembagaModal'));
        if (modal) {
            modal.hide();
        }
    });
