@section('title', __('Manajemen Program Studi'))
...
<div class="row g-4">
    <div class="col-lg-12">
        ...
        <!-- Modal Form -->
        <div class="modal fade" id="prodiModal" tabindex="-1" wire:ignore.self>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ $isEditMode ? __('Edit Program Studi') : __('Tambah Program Studi') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        ...
                        <form wire:submit.prevent="save">
                            <div class="row g-3">
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        <label for="nama_prodi" class="form-label fw-medium">{{ __('Nama Program Studi') }} <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('form.nama_prodi') is-invalid @enderror" 
                                               id="nama_prodi" wire:model="form.nama_prodi" placeholder="{{ __('Masukkan nama program studi') }}">
                                        @error('form.nama_prodi') 
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ __('Konfirmasi Hapus Program Studi') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        <p>{{ __('Yakin ingin menghapus program studi ini? Tindakan ini tidak dapat dibatalkan.') }}</p>
                    </div>
                    <div class="modal-footer">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('Daftar Program Studi') }}</h5>
                <div class="d-flex gap-2">
                    <!-- Search -->
                    <div class="position-relative">
                        <input type="text" 
                               class="form-control" 
                               placeholder="{{ __('Cari program studi...') }}"
                    ...
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            @forelse ($prodis as $index => $prodi)
                                <tr>
                                    <td class="fw-medium">{{ $prodis->firstItem() + $index }}</td>
                                    <td>{{ $prodi->nama_prodi }}</td>
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <button class="btn btn-sm btn-primary" wire:click="edit({{ $prodi->id }})" title="{{ __('Edit') }}">
                                                <i class="bx bx-edit-alt"></i>
                                            </button>
                                            <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $prodi->id }})" title="{{ __('Hapus') }}">
                                ...
                                <tr>
                                    <td colspan="3" class="text-center py-5">
                                        <div class="text-muted">
                                            ...
                                            @else
                                                <p class="mb-0">{{ __('Belum ada program studi yang ditambahkan') }}</p>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
...
document.addEventListener('livewire:init', () => {
    ...
    Livewire.on('hide-modal', () => {
        const modal = bootstrap.Modal.getInstance(document.getElementById('prodiModal'));
        if (modal) {
            modal.hide();
        }
    });
