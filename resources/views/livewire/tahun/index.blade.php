@section('title', __('Manajemen Tahun'))
...
<div class="row g-4">
    <div class="col-lg-12">
        ...
        <!-- Modal Form -->
        <div class="modal fade" id="tahunModal" tabindex="-1" wire:ignore.self>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ $isEditMode ? __('Edit Tahun') : __('Tambah Tahun') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        ...
                        <form wire:submit.prevent="save">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="tahun" class="form-label fw-medium">{{ __('Tahun') }} <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control @error('form.tahun') is-invalid @enderror" 
                                               id="tahun" wire:model="form.tahun" placeholder="{{ __('Masukkan tahun') }}">
                                        @error('form.tahun') 
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ __('Konfirmasi Hapus Tahun') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        <p>{{ __('Yakin ingin menghapus tahun ini? Tindakan ini tidak dapat dibatalkan.') }}</p>
                    </div>
                    <div class="modal-footer">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('Daftar Tahun') }}</h5>
                <div class="d-flex gap-2">
                    <!-- Search -->
                    <div class="position-relative">
                        <input type="text" 
                               class="form-control" 
                               placeholder="{{ __('Cari tahun...') }}"
                    ...
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            @forelse ($tahuns as $index => $tahun)
                                <tr>
                                    <td class="fw-medium">{{ $tahuns->firstItem() + $index }}</td>
                                    <td>{{ $tahun->tahun }}</td>
                                    <td>
                                        <span class="badge {{ $tahun->status === 'aktif' ? 'bg-success' : 'bg-secondary' }}">
                                            {{ $tahun->status === 'aktif' ? __('Aktif') : __('Nonaktif') }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <button class="btn btn-sm btn-primary" wire:click="edit({{ $tahun->id }})" title="{{ __('Edit') }}">
                                                <i class="bx bx-edit-alt"></i>
                                            </button>
                                            <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $tahun->id }})" title="{{ __('Hapus') }}">
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        <div class="text-muted">
                                            ...
                                            @else
                                                <p class="mb-0">{{ __('Belum ada tahun yang ditambahkan') }}</p>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
...
document.addEventListener('livewire:init', () => {
    ...
    Livewire.on('hide-modal', () => {
        const modal = bootstrap.Modal.getInstance(document.getElementById('tahunModal'));
        if (modal) {
            modal.hide();
        }
    });
