@section('title', __('Manajemen Jabatan'))
...
<div class="row g-4">
    <div class="col-lg-12">
        ...
        <!-- Modal Form -->
        <div class="modal fade" id="jabatanModal" tabindex="-1" wire:ignore.self>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ $isEditMode ? __('Edit Jabatan') : __('Tambah Jabatan') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        ...
                        <form wire:submit.prevent="save">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="jabatan" class="form-label fw-medium">{{ __('Jabatan') }} <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('form.jabatan') is-invalid @enderror" 
                                               id="jabatan" wire:model="form.jabatan" placeholder="{{ __('Masukkan nama jabatan') }}">
                                        @error('form.jabatan') 
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="unit" class="form-label fw-medium">{{ __('Unit') }} <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control @error('form.unit') is-invalid @enderror" 
                                               id="unit" wire:model="form.unit" placeholder="{{ __('Masukkan unit') }}">
                                        @error('form.unit') 
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            ...
                            {{ __('Konfirmasi Hapus Jabatan') }}
                        </h5>
                    </div>
                    <div class="modal-body">
                        <p>{{ __('Yakin ingin menghapus jabatan ini? Tindakan ini tidak dapat dibatalkan.') }}</p>
                    </div>
                    <div class="modal-footer">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ __('Daftar Jabatan') }}</h5>
                <div class="d-flex gap-2">
                    <!-- Search -->
                    <div class="position-relative">
                        <input type="text" 
                               class="form-control" 
                               placeholder="{{ __('Cari jabatan atau unit...') }}"
                    ...
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            @forelse ($jabatans as $index => $jabatan)
                                <tr>
                                    <td class="fw-medium">{{ $jabatans->firstItem() + $index }}</td>
                                    <td>{{ $jabatan->jabatan }}</td>
                                    <td>{{ $jabatan->unit }}</td>
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center">
                                            <button class="btn btn-sm btn-primary" wire:click="edit({{ $jabatan->id }})" title="{{ __('Edit') }}">
                                                <i class="bx bx-edit-alt"></i>
                                            </button>
                                            <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $jabatan->id }})" title="{{ __('Hapus') }}">
                                ...
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        <div class="text-muted">
                                            ...
                                            @else
                                                <p class="mb-0">{{ __('Belum ada jabatan yang ditambahkan') }}</p>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
